...
class _AddStructureScreenState extends State<AddStructureScreen> {
  ...
  void initState() {
    ...
    _nomFocusNode.addListener(() {
      if (!_nomFocusNode.hasFocus && nomCtrl.text.trim().isNotEmpty) {
  ...
  void dispose() {
    ...
    disponibiliteCtrl.dispose();
    rueCtrl.dispose();
    codePosteCtrl.dispose();
    geoLocCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _loadInitialData() async {
    try {
      ...
      if (mounted) setState(() { isTypesLoading = false; isVillesLoading = false; });
    }
  }
  ...
  Future<void> _checkNameAvailability(String nom) async {
    ...
    try {
      final response = await http.get(Uri.parse('$baseUrl/structure/exists?nom=${Uri.encodeComponent(nom)}'));
      if (!mounted) return;
      if (response.statusCode == 200) {
        setState(() => nameError = jsonDecode(response.body) ? "Ce nom est déjà utilisé ❌" : null);
      }
    } catch (e) {
      debugPrint("Erreur validation nom: $e");
    } finally {
      if (mounted) setState(() => isCheckingName = false);
    }
  }
  ...
  Future<void> _getCurrentLocation() async {
    ...
    try {
      ...
      if (mounted) setState(() => geoLocCtrl.text = "${position.latitude}, ${position.longitude}");
    } catch (e) {
